...
//Code was further edited by Parnak.N.Dave
...
int game_dijkstra_search(const game_info_t* info,
	...
	while(result == SEARCH_IN_PROGRESS){
		...
		node_next = heapq_deque(&pq);
		assert(node_next);

		//Get next color to explore its 4 direction
	      //(use game_next_move_color function in engine.h)
		game_state_t* current_state = &node_next->state;
		...
		for(int direction = 0; direction < 4; direction++){
			...
			if(game_can_move(info, &node_next->state,color, direction)){

				//Create child node
				tree_node_t* child = node_create(node_next, info, current_state);
				assert(child);
			...
			//In no more space in memory, end search (more nodes in pq than max_nodes)
				...
				game_make_move(info, &child->state, color, direction);
